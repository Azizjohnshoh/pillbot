...
async def webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if "message" in data:
        ...
        if text.startswith("/start"):
            ...
            info_text = (
                "💊 Dori qo‘shish funksiyasi ishlamoqda.\n"
                "Iltimos, ma'lumot kiriting (masalan: Paracetamol, 08:00, har kuni)"
            )
            background_tasks.add_task(send_message, chat_id, info_text)
            return {"ok": True}
    ...
    if "callback_query" in data:
        ...
        if data_payload == "add_med":
            background_tasks.add_task(
                send_message,
                chat_id,
                "💊 Dori qoʻshish funksiyasi — iltimos nom, vaqt va interval kiriting.",
            )
            ...
            if meds:
                text = "Sizning dorilaringiz:\n" + "\n".join(
                    [f"{r['id']}: {r['title']} @ {r['time']}" for r in meds]
                )
            else:
            ...
            background_tasks.add_task(send_message, chat_id, "⚙️ Sozlamalar: ovoz, vaqt zonasi, til.")
